...
describe MaxMindDB do
  ...
  [
    ['Default', MaxMindDB::DEFAULT_FILE_READER],
    ['Mmap', proc { |path| Mmap.new(path, 'r') }]
  ].each do |mode|
    name, file_reader = mode

    describe "file_reader=#{name}" do

      let(:city_db) { MaxMindDB.new('spec/cache/GeoLite2-City.mmdb', file_reader) }
      let(:country_db) { MaxMindDB.new('spec/cache/GeoLite2-Country.mmdb', file_reader) }
      let(:rec32_db) { MaxMindDB.new('spec/data/32bit_record_data.mmdb', file_reader) }
